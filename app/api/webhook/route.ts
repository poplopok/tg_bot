...
async function analyzeEmotion(text: string): Promise<EmotionAnalysis> {
  ...
  console.log(
    `[АНАЛИЗ] Текст: "${text.substring(0, 50)}..." | Режим: ${modelPreference}`
  );
...
function getNeutralResult(text: string, error?: Error): EmotionAnalysis {
  console.log(
    `[НЕЙТРАЛЬНЫЙ РЕЗУЛЬТАТ] ${
      error ? `Ошибка: ${error.message}` : "Анализ отключен"
    }`
  );
...
<b>Режим работы:</b> ${
    modelInfo === "ai" || modelInfo === "advanced"
      ? "🧠 Только AI модели"
      : "⏸️ Анализ отключен"
  }
...
bot.command("fix_models", async (ctx) => {
  ...
  await ctx.reply(
    "✅ Модели переключены. Пожалуйста, перезапустите бота для применения изменений."
  );
});
...
bot.command("health", async (ctx) => {
  ...
  try {
    ...
    console.log(
      `[HEALTH CHECK] HUGGINGFACE_API_KEY установлен: ${!!process.env
        .HUGGINGFACE_API_KEY}`
    );
    ...
    console.log(
      `[HEALTH CHECK] Результат анализа:`,
...
🔑 <b>API ключ:</b> ${
      process.env.HUGGINGFACE_API_KEY ? "✅ Установлен" : "❌ Отсутствует"
    }
...
${
  analysis.correctedText !== testPhrase
    ? `✏️ Исправлено: "${escapeHtml(analysis.correctedText || "")}"`
    : "✏️ Исправлений не требуется"
}
${
  analysis.normalizedText !== testPhrase
    ? `🔄 Нормализовано: "${escapeHtml(analysis.normalizedText || "")}"`
    : "🔄 Нормализация не требуется"
}

${
  analysis.slangDetected && analysis.slangDetected.length > 0
    ? `🗣️ Обнаружен сленг: ${analysis.slangDetected.join(", ")}`
    : "🗣️ Сленг не обнаружен"
}
${
  analysis.errorsFixed && analysis.errorsFixed.length > 0
    ? `✏️ Исправлены ошибки: ${analysis.errorsFixed.join(", ")}`
    : "✏️ Ошибок не найдено"
}
...
bot.command("debug_models", async (ctx) => {
  await ctx.reply(
    "🔍 Тестирую модель j-hartmann/emotion-english-distilroberta-base..."
  );
  ...
  debugReport += `🔑 <b>API ключ:</b> ${
    process.env.HUGGINGFACE_API_KEY
      ? `✅ Есть (${process.env.HUGGINGFACE_API_KEY.substring(0, 8)}...)`
      : "❌ ОТСУТСТВУЕТ"
  }\n`;
  ...
  try {
    ...
    console.log(
      `[DEBUG MODEL] ${model.name} - Результат:`,
    ...
    if (response.status === 200) {
      ...
      console.log(
        `[DEBUG MODEL] Raw result is array: ${Array.isArray(result)}`
      );
      console.log(
        `[DEBUG MODEL] Raw result length: ${
      ...
      if (Array.isArray(result)) {
        result.forEach((item, index) => {
          console.log(
            `[DEBUG MODEL] Item ${index}:`,
            JSON.stringify(item, null, 2)
          );
          console.log(`[DEBUG MODEL] Item ${index} keys:`, Object.keys(item));
          console.log(
            `[DEBUG MODEL] Item ${index} values:`,
            Object.values(item)
          );
        });
      } else {
        console.log(
          `[DEBUG MODEL] Non-array result keys:`,
          Object.keys(result)
        );
        console.log(
          `[DEBUG MODEL] Non-array result values:`,
      ...
      if (Array.isArray(result) && result.length > 0) {
        ...
        result.slice(0, 5).forEach((emotion: any, index: number) => {
          console.log(`[DEBUG MODEL] Processing emotion ${index}:`, emotion);
          ...
          const label =
            possibleLabels.find((l) => l !== undefined) || "unknown";
          const score = possibleScores.find((s) => s !== undefined) || 0;

          console.log(
            `[DEBUG MODEL] Extracted - Label: ${label}, Score: ${score}`
          );

          debugReport += `      • ${label}: ${(score * 100).toFixed(1)}%\n`;
          debugReport += `      • Raw data: ${JSON.stringify(emotion).substring(
            0,
            100
          )}...\n`;
        });
        ...
        debugReport += `   📋 Неожиданный формат результата:\n`;
        debugReport += `   📋 Тип: ${typeof result}\n`;
        debugReport += `   📋 Содержимое: ${JSON.stringify(result).substring(
          0,
          200
        )}...\n`;
      }
    ...
    try {
      ...
      debugReport += `   🎯 Эмоция: ${
        localResult.emotion
      } (${localResult.confidence.toFixed(1)}%)\n`;
      debugReport += `   😡 Агрессия: ${localResult.categories.aggression.toFixed(
        1
      )}%\n`;
      debugReport += `   ☣️ Токсичность: ${localResult.categories.toxicity.toFixed(
        1
      )}%\n`;
    } catch (localError) {
    ...
    debugReport += `   ❌ Критическая ошибка: ${escapeHtml(
      error.toString()
    )}\n\n`;
  }

  debugReport += `\n🕐 <b>Тестирование завершено:</b> ${new Date().toLocaleString(
    "ru-RU"
  )}`;
...
bot.command("test", async (ctx) => {
  ...
  for (const phrase of testPhrases) {
    try {
      ...
      testResults += `   😡 Агрессия: ${analysis.categories.aggression.toFixed(
        1
      )}%\n`;
      testResults += `   ☣️ Токсичность: ${analysis.categories.toxicity.toFixed(
        1
      )}%\n`;
      testResults += `   🤖 AI модели: ${analysis.modelUsed.join(", ")}\n`;
      testResults += `   ⏱️ Время: ${processingTime}ms\n\n`;
    } catch (error) {
      testResults += `❌ Ошибка AI анализа "${escapeHtml(
        phrase
      )}": ${error}\n\n`;
    }
  }
...
${
  Array.from(stats.userRisks.values())
    ...
    .map(
      (user) =>
        `⚠️ @${user.username || user.userId} (${user.incidents} AI-инцидентов)`
    )
...
bot.on("message:text", async (ctx) => {
  ...
  try {
      ...
      if (isIncident) {
        ...
        if (
          ...
          try {
            ...
            let moderationMessage = `⚠️ Сообщение удалено AI-модератором (токсичность: ${Math.round(
              analysis.categories.toxicity
            )}%, уверенность: ${Math.round(analysis.confidence)}%).`;

            if (analysis.slangDetected && analysis.slangDetected.length > 0) {
              moderationMessage += `\n\n🗣️ AI обнаружил неуместный сленг: ${analysis.slangDetected
                .slice(0, 3)
                .join(", ")}`;
            }
        ...
        if (analysis.severity === "high") {
          let warningMessage = `⚠️ @${
            username || userId
          }, AI анализ показал негативную тональность сообщения. Давайте поддерживать позитивную атмосферу в команде! 😊`;

          if (analysis.slangDetected && analysis.slangDetected.length > 0) {
            warningMessage += `\n\n💡 AI совет: избегайте сленга в рабочем общении.`;
          }

          warningMessage += `\n\n🤖 Уверенность AI: ${Math.round(
            analysis.confidence
          )}%`;
    ...
    console.log(
      `[AI АНАЛИЗ ${new Date().toISOString()}] Chat: ${chatId}, User: ${userId}, Emotion: ${
        analysis.emotion
      }, Confidence: ${
        analysis.confidence
      }%, AI Models: ${analysis.modelUsed?.join(",")}${
        analysis.slangDetected && analysis.slangDetected.length > 0
          ? `, AI Slang: ${analysis.slangDetected.length}`
          : ""
      }${
        analysis.errorsFixed && analysis.errorsFixed.length > 0
          ? `, AI Errors: ${analysis.errorsFixed.length}`
          : ""
      }`
    );
  } catch (error) {
