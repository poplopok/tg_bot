...
export async function GET() {
  try {
    const apiKey =
      ...
      "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";
    ...
    try {
      ...
      try {
        ...
        return NextResponse.json({
          ...
          message:
            "API ключ работает, но русская модель недоступна. Используем альтернативную модель.",
          api_key_valid: true,
      ...
      return NextResponse.json(
        {
          ...
          suggestion:
            "API ключ неверный или истек. Создайте новый на https://huggingface.co/settings/tokens",
        },
        { status: 401 }
      );
    }
