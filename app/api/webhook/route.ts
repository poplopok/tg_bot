...
bot.command("debug_models", async (ctx) => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      console.log(`[DEBUG MODEL] Raw result is array: ${Array.isArray(result)}`)
      console.log(`[DEBUG MODEL] Raw result length: ${Array.isArray(result) ? result.length : "N/A"}`)

      if (Array.isArray(result)) {
        result.forEach((item, index) => {
          console.log(`[DEBUG MODEL] Item ${index}:`, JSON.stringify(item, null, 2))
          console.log(`[DEBUG MODEL] Item ${index} keys:`, Object.keys(item))
          console.log(`[DEBUG MODEL] Item ${index} values:`, Object.values(item))
        })
      } else {
        console.log(`[DEBUG MODEL] Non-array result keys:`, Object.keys(result))
        console.log(`[DEBUG MODEL] Non-array result values:`, Object.values(result))
      }
      ...
      if (Array.isArray(result) && result.length > 0) {
        ...
        result.slice(0, 5).forEach((emotion: any, index: number) => {
          ...
          const possibleLabels = [emotion?.label, emotion?.LABEL, emotion?.emotion, emotion?.class, emotion?.category]
          ...
          const label = possibleLabels.find((l) => l !== undefined) || "unknown"
          const score = possibleScores.find((s) => s !== undefined) || 0

          console.log(`[DEBUG MODEL] Extracted - Label: ${label}, Score: ${score}`)

          debugReport += `      • ${label}: ${(score * 100).toFixed(1)}%\n`
          debugReport += `      • Raw data: ${JSON.stringify(emotion).substring(0, 100)}...\n`
        })
        ...
        debugReport += `   📋 Содержимое: ${JSON.stringify(result).substring(0, 200)}...\n`
      }
    }
