...
• 😊 RuBERT-CEDR - анализ эмоций (RU)
• 😊 DistilRoBERTa - анализ эмоций (EN)
• 😏 RoBERTa-Irony - детекция сарказма
• 🗣️ Custom Slang DB - 5000+ выражений
...
*Компоненты:*
• Словарные анализаторы
• Анализ пунктуации и эмодзи
• Детекция CAPS и повторов
• Базовая нормализация сленга
...
• Базовый анализ эмоций`,
  ...
  const stats = {
    advanced: { accuracy: "92-95%", speed: "2-4 сек", languages: "RU, EN, DE, FR", slang: "5000+" },
    local: { accuracy: "70-75%", speed: "<1 сек", languages: "RU, EN", slang: "500+" },
  }

  const currentStats = stats[modelInfo as keyof typeof stats] || stats.advanced

  await ctx.reply(
    `${currentModel}

*Статистика обработки:*
• Точность: ${currentStats.accuracy}
• Скорость: ${currentStats.speed}
• Языки: ${currentStats.languages}
• База сленга: ${currentStats.slang} выражений

*Текущий режим:* ${modelInfo === "advanced" ? "🧠 Продвинутый" : "💻 Локальный"}
